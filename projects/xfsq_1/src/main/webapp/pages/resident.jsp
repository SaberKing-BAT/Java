...
<div class="x-body">
            ...
            <div class="layui-form-item">
                <div class="layui-input-inline layui-bg-cyan"  style="width: 120px">
                    <select lay-verify="required" id="tiaojian" name="tiaojian">
                        ...
                        <option value="6">户籍</option>
                        <option value="7">入住时间</option>
                    </select>
                </div>
    ...
    <xblock>
        <button class="layui-btn" onclick="person_add('添加信息','${pageContext.request.contextPath}/goAddResident','400','500')"><i class="layui-icon">&#xe608;</i>添加</button>
...
<script>
    ...
    function showList(data){
        ...
        for(var i in data){
            ...
            tableHtml += "<tr>";
            tableHtml += "<td>"+" <input type='checkbox' value="+data[i].jid+" name='checkbox1' id='resident_id'>" + "</td>";
            tableHtml +=    "<td>"+data[i].jid + "</td>";
            tableHtml +=    "<td>"+data[i].jname + "</td>";
            tableHtml +=    "<td>"+data[i].jsex + "</td>";
            tableHtml +=    "<td>"+data[i].jage + "</td>";
            tableHtml +=    "<td>"+data[i].jphone + "</td>";
            tableHtml +=    "<td>"+data[i].jadress + "</td>";
            tableHtml +=    "<td>"+data[i].idadress + "</td>";
            tableHtml +=    "<td>"+data[i].intime + "</td>";
            tableHtml +=    "<td>"+data[i].jpw + "</td>";
            tableHtml +=  " <td class='td-manage'>"
                +"<a title='修改' href='javascript:;' onclick='person_look(&quot;修改&quot; ,&quot;goResidentUpdate &quot;, "+resident+",&quot;400&quot;,&quot;500&quot;)' class='ml-5' style='text-decoration:none'>"
    ...
    function x_admin_showm(title, url, data, w, h) {
        ...
        layer.open({
            ...
            success:function (layero, index) {
                ...
                body.contents().find("#jid").val(data.jid);
                body.contents().find("#jname").val(data.jname);
                body.contents().find("#jsex").val(data.jsex);
                body.contents().find("#jage").val(data.jage);
                body.contents().find("#jphone").val(data.jphone);
                body.contents().find("#jadress").val(data.jadress);
                body.contents().find("#idadress").val(data.idadress);
                body.contents().find("#L_title").val(data.intime);
                body.contents().find("#jpw").val(data.jpw);
            }
        })
    }
    //条件查询
    $("#sousuo").click(function() {
        /*alert($('#sousuoform').serialize())*/
        $.ajax({
            type: 'get',
            url: "${pageContext.request.contextPath}/ifResident",
            cache: false,
